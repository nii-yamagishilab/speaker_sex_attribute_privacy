...
class Generator(torch.nn.Module):
    ...
    def forward(self, x, xvect):#, f0, content, xv):
...
class latentGenerator(Generator):
    def __init__(self, h):
        ...
        if h.get("xv_disentanglement", None) and h.get("xv_disentanglement_model", None):
            self.xv_dis         = True
            self.xv_dis_model   = torch.load(str(h.xv_disentanglement_model))
            set_parameter_requires_grad(self.xv_dis_model, True)
            #self.xvm            = torch.Tensor(np.loadtxt(h.xvmean))
            #self.xvs            = torch.Tensor(np.loadtxt(h.xvstd))
            if h.get("xv_protect_dis", None):
                self.xv_protect = 'dis'
            elif h.get("xv_protect_glob", None):
                self.xv_protect = 'glob'
            else:
                self.xv_protect = None
        elif (not h.get("xv_disentanglement", None)) and h.get("xv_disentanglement_model", None):
            self.xv_dis = False
            self.xv_dis_model   = torch.load(str(h.xv_disentanglement_model))
            set_parameter_requires_grad(self.xv_dis_model, True)
            if h.get("xv_protect_dis", None):
 ...
 #_______
            if self.xv_dis == True:
                xv = xv.squeeze(dim=1)
                xv = torch.nn.functional.normalize(xv,dim=1)
                #xv = (xv-(self.xvm).to(xv.device))/(self.xvs).to(xv.device)
                xvd, _ = self.xv_dis_model.f(xv)
                if self.xv_protect == True:
                    xvd[:,0] = 0
                xv = xvd.unsqueeze(1)
            
            elif self.xv_protect == True:
                xv = xv.squeeze(dim=1) 
                xvd, _ = self.xv_dis_model.f(xv)
                xvd[:,0] = 0
                xv = self.xv_dis_model.g(xvd)
                xv = xv.unsqueeze(1)
            #xvect = xv
    ...
    def gen_vpc(self, xv_path,**kwargs):
        ...
        if self.latent and self.xv:
            ...
            if self.xv_protect == 'glob':
                # extract xvector
                # read anon xvector
                xv = np.loadtxt(xv_path)
                xv = torch.FloatTensor(xv).unsqueeze(0).unsqueeze(0).to(x.device) 
...
#________           
 ...
 #_______
            if self.xv_dis == True:
                ...
                xv = torch.nn.functional.normalize(xv,dim=1)
                xvd, _ = self.xv_dis_model.f(xv)
                if self.xv_protect == 'dis':
                    xvd[:,0] = 0
                xv = xvd.unsqueeze(1)
            elif self.xv_protect == 'dis':
                xv = xv.squeeze(dim=1) 
                xvd, _ = self.xv_dis_model.f(xv)
                xvd[:,0] = 0
                xv = self.xv_dis_model.g(xvd)
        ...
        return super().forward(x, xvect)#,f0,content,xvect)
