...
class latentDataset(torch.utils.data.Dataset):
    def __init__(self, training_files, segment_size, n_fft, num_mels,
                 ...
                 device=None, fmax_loss=None, f0=True, f0conv=False):
    ...
    def __getitem__(self, index):
        ...
        if self.f0:
            ...
            if self.f0conv:
                targ_f0_mean = 154.37  #164.78
                targ_f0_std = 30.2
                #targ_lf0_mean   = 5.040557
                #targ_lf0_std    = 0.1833
                mask = f0 != 0
                if np.sum(mask)>0:
                    ...
                    sf0 = np.std(f0[mask])
                    f0[mask] = f0[mask] - mf0
                    f0[mask] = targ_f0_std*f0[mask]/sf0
                    f0[mask] = f0[mask] + targ_f0_mean
                   

                    #AFFINE TRANFORMATION IN THE LOG DOMAIN

                    #mlf0 = np.mean(np.log(f0[mask]))
                    #slf0 = np.std(np.log(f0[mask]))
                    #f0[mask] = np.exp(np.log(f0[mask]) - mlf0)
                    #f0[mask] = np.exp(targ_lf0_std*np.log(f0[mask])/slf0)
                    #f0[mask] = np.exp(np.log(f0[mask]) + targ_lf0_mean)
