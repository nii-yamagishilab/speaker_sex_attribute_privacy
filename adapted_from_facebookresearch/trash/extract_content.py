...
from dataset_test_ import latentDataset, mel_spectrogram, \
    MAX_WAV_VALUE
from utils import AttrDict
from models_test_ import latentGenerator,SoftPredictor
import joblib
import fairseq
...
def extract_cont(item_index):
    x, filename = dataset[item_index]
    x = {k: torch.autograd.Variable(v.to(device, non_blocking=False)) for k, v in x.items()}
    fname_out_name = Path(filename).stem
    xv_path = str(a.xv_dir) + '/' + fname_out_name + '.xvector'
    c = generate(h, generator, x, xv_path)
    output_file = os.path.join(a.output_dir, fname_out_name + '.cont')
    np.savetxt(output_file,c.cpu().detach().numpy(),fmt='%.6e')
