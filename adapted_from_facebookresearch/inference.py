...
# LICENSE file in the root /irectory of this source tree.
...
def generate(h, generator, x, xv_path):
    start = time.time()
    #y_g_hat, f0, content, xv = generator.gen_vpc(xv_path,**x)
    y_g_hat, _ = generator.gen_vpc(xv_path,**x)
    y_g_hsat = y_g_hat.to(device)
    ...
    return audio, rtf#, f0, content, xv
...
def init_worker(arguments):
    ...
    dataset = latentDataset(file_list, -1, h.n_fft, h.num_mels, h.hop_size, h.win_size,
                              ...
                              fmax_loss=h.fmax_for_loss,  device=device, f0conv=h.f0conv)
...
def inference(item_index):
    ...
    #f0      = x['f0'].reshape((1,x['f0'].shape[2]))
    #f0ori   = x['f0ori']
        
    #np.savetxt("/home/smg/v-noepg/SSL-SAS/targetf0/"+filename+'.f0', f0.cpu().numpy(), fmt='%.4e')
    #np.savetxt("/home/smg/v-noepg/SSL-SAS/targetf0/"+filename+'.f0ori', f0ori.cpu().numpy(), fmt='%.4e')
    ...
    xv_glob = str(a.xv_glob)
    
    #audio, rtf, f0, content, xv = generate(h, generator, x, xv_path)
    audio, rtf = generate(h, generator, x, xv_glob)    
    output_file = os.path.join(a.output_dir, fname_out_name + '.wav')
    #np.savetxt(a.output_dir + '/' + fname_out_name + '.f0', f0.squeeze().cpu().numpy(),fmt='%.6e')
    #np.savetxt(a.output_dir + '/' + fname_out_name + '.c', content.squeeze().cpu().numpy(),fmt='%.6e')
    #np.savetxt(a.output_dir + '/' + fname_out_name + '.xv', xv.squeeze().cpu().numpy(),fmt='%.6e')
...
def main():
    ...
    parser.add_argument('--xv_glob', type=Path)
