...
#__________________________________
    ...
    #Xvector extrator !!!!!
    if h.get('consistency_loss', None):
        if h.get('ecapa_fbank_model_path', None):
            ...
            xv_model = ECAPA_TDNN(80, lin_neurons=192, out_neurons=5994)
            pre_train_model = torch.load(str(h.ecapa_fbank_model_path))
            xv_model.load_state_dict(pre_train_model)
            xv_model = xv_model.to(device)
            set_parameter_requires_grad(xv_model, h.get('xv_freeze', None))
        else:
            consistency_loss = False
            print("Consistency loss ignored")
    else:
        consistency_loss = False


    #Xvector disentanglement !!!!!!!
    if h.get("xv_disentanglement", None) and h.get("xv_disentanglement_model", None):
        xv_dis          = True
        xv_dis_model    = torch.load(str(h.xv_disentanglement_model))
        xv_dis_model    = xv_dis_model.to(device)
        set_parameter_requires_grad(xv_dis_model, True)
        #self.xvm            = torch.Tensor(np.loadtxt(h.xvmean))
        #self.xvs            = torch.Tensor(np.loadtxt(h.xvstd))
    else:
        xv_dis = False
...
#__________________________________
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            y_g_hat, target_xvect = generator(**x)
            ...
            if consistency_loss == True:
                xv_input        = fbank(y_g_hat.squeeze(1))
                xv_input        = mean_var_norm(xv_input, torch.ones(xv_input.shape[0]).to(xv_input.device))
                xv_hat, _   = xv_model(xv_input)
                if xv_dis == True:
                    xv_hat = xv_hat.squeeze(dim=1)
                    xv_hat = torch.nn.functional.normalize(xv_hat,dim=1)
                    xv_hat, _   = xv_dis_model.f(xv_hat)
                    xv_hat      = xv_hat.unsqueeze(1)
                
                loss_xvect = torch.linalg.norm((target_xvect - xv_hat.to(device)).squeeze(), dim=1).mean()
            ...
            #loss_gen_all_noxv = loss_gen_all
            if consistency_loss == True:
                loss_gen_all = loss_gen_all + a.lambda_xv*loss_xvect
            #__________________________
            ...
            if rank == 0:
                ...
                if steps % a.summary_interval == 0:
                    ...
                    if consistency_loss == True:
                        sw.add_scalar("training/xv_loss", loss_xvect, steps)
                ...
                if steps % a.validation_interval == 0:  # and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            #if consistency_loss == True:
                            #    xv_input        = fbank(y_g_hat.squeeze(1).detach().cpu())
                            #    xv_input        = mean_var_norm(xv_input, torch.ones(xv_input.shape[0]).to(xv_input.device))
                            #    xv_hat, _   = xv_model(xv_input)
                            #    if xv_dis == True:
                            #        xv_hat = xv_hat.squeeze(dim=1)
...
def main():
    ...
    parser.add_argument('--lambda_xv', type=float, default=1.0)
