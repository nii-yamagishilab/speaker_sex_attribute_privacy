...
class Generator(torch.nn.Module):
    ...
    def forward(self, x):
        c = torch.mean(x,dim=1).reshape((1,200))
        #c = torch.mean(x.squeeze()[0:200,:],dim=1).reshape((1,200))
        return c
    ...
    def forward(self, **kwargs):
        ...
        if self.latent and self.xv:
            if self.ssl_type == 'hubert_soft':
                ...
                #C = torch.mm((x-xm).T, x-xm)
                #C = C/x.shape[0]

                #print(C)

                #eigv, eigV = torch.eig(C, eigenvectors=True)
                
                #eigv    = eigv[:,0]
                #Cinvrs = torch.mm(eigV, torch.diag(1/torch.sqrt(eigv)))

                #x = torch.mm(Cinvrs, (x-xm).T).T
                #x = x.unsqueeze(0)
    ...
    def gen_vpc(self, xv_path,**kwargs):
        ...
        if self.latent and self.xv:
            if self.ssl_type == 'hubert_soft':
                ...
                x = self.latent_encoder(audio_data)
                #x = self.latent_encoder(audio_data).squeeze()
                #xm = x.mean(dim=0)
