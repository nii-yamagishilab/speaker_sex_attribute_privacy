...
if [ -e "/Database/LibriTTS/train-clean-100/" ];then
    ### single gpu setting ###
    ### train libri HiFi-GAN
    export CUDA_VISIBLE_DEVICES="0,1,2"
    python3 -m torch.distributed.launch --nproc_per_node 2 adapted_from_facebookresearch/train.py \
	    --checkpoint_path checkpoints/HifiGAN_train \
    --config configs/train_hifigan.json
